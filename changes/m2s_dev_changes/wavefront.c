...
void SIWavefrontCreate(SIWavefront *self, int id, SIWorkGroup *work_group)
{
	...
	struct SIAsmWrap *as = emu->as;   //associated dissassmebler
...
/*MIAOW Start */
void SIWavefrontDump(SIWavefront *self, FILE *f)
{
	SINDRange *ndrange = self->ndrange;
	SIWorkGroup *work_group = self->work_group;
...
void SIWavefrontExecute(SIWavefront *self)
{
	...
	if (debug_status(si_isa_debug_category))  //category is 1 if --si-debug-isa is specified
	{
		si_isa_debug("\n");
