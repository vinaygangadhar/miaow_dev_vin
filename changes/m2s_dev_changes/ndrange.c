...
/*MIAOW stop */
	...
	//Overwrite the default values and passed parameters with config.txt values
...
//UNIT TEST
	...
	if(uni_test_config!= NULL){
		...
		if(config_read_result != NULL){
			...
			self->group_count = atoi(tok);

			tok = strtok(NULL, ";"); //Total Work items count
			num_of_threads = atoi(tok);
			
			self->global_size = atoi(tok);
			self->global_size3[0] = atoi(tok);
			self->local_size3[0] = atoi(tok);
		}
	}
			
	//Workgroup Count and thread count
	fprintf(config, "%d;%d;\n", self->group_count, self->global_size);
	...
	for(int vreg_init_indexi = 0; vreg_init_index < 256; vreg_init_index++){
