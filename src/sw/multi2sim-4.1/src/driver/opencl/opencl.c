...
static int opencl_abi_si_kernel_launch_impl(struct x86_ctx_t *ctx)
{
	...
	if (unit_test_config != 0)
	{
		...
		config_read_result = fgets(unit_test_input_buf, 1500, unit_test_config);
		if(config_read_result != NULL)
		{
			...
			global_size[0] = num_of_threads;
			local_size[0] = num_of_threads;
		}
