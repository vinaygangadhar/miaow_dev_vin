...
int setSregSrc() {
  return (rand() % (configs.scalar_reg/2)) + configs.scalar_reg/2 ;
}

int setSregDest() {
  return rand() % (configs.scalar_reg/2);
}
...
void randomizeOperand() {
	...
	opvals.vdest = (rand() % (configs.vector_reg/2));
	opvals.sbase = ((rand() % (configs.scalar_reg/2))  + (configs.scalar_reg/2))/2;
	opvals.sbase2 = ((rand() % (configs.scalar_reg/2) + (configs.scalar_reg/2) )/4);

	// conservative choice for use in smrd/mtbuf
	opvals.offset = (rand() % (configs.scalar_reg/2)) + (configs.scalar_reg/2);
	...
	if(s_or_v1 == 0) {
	  ...
	  opvals.src0 = 256 + (rand() % (configs.vector_reg/2)) + (configs.vector_reg/2);
	}
	...
	if(s_or_v2 == 0) {
	  ...
	  opvals.src1 = 256 + (rand() % (configs.vector_reg/2)) + (configs.vector_reg/2);
	}
	...
	if(s_or_v3 == 0) {
	  ...
	  opvals.src2 = 256 + (rand() % (configs.vector_reg/2)) + (configs.vector_reg/2);
	}

	opvals.vsrc1 = (rand() % (configs.vector_reg/2)) + (configs.vector_reg/2);
	opvals.vsrc2 = (rand() % (configs.vector_reg/2)) + (configs.vector_reg/2);
...
void instruction_sop1(int opcode) {
	...
	instr.sop1.ssrc0 = (opcode == 0x24 || opcode == 0x04) ? 
	  (((rand()%(configs.scalar_reg/2))+(configs.scalar_reg/2))/2)*2 : opvals.ssrc0;
	instr.sop1.op = opcode;
	instr.sop1.sdst = (opcode == 0x24 || opcode == 0x04) ? 
	  ((rand()%(configs.scalar_reg/2))/2)*2 : opvals.sdest;
...
void instruction_sop2(int opcode) {
	...
	instr.sop2.ssrc0 = (opcode == 0x15 || opcode == 0x0F) ?  
	  (((rand()%(configs.scalar_reg/2)) + (configs.scalar_reg/2) )/2)*2 : opvals.ssrc0;
	instr.sop2.ssrc1 = (opcode == 0x15 || opcode == 0x0F) ? 
	  (((rand()%(configs.scalar_reg/2)) + (configs.scalar_reg/2) )/2)*2 : opvals.ssrc1;
	instr.sop2.sdst = (opcode == 0x15 || opcode == 0x0F) ? 
	  (((rand()%(configs.scalar_reg/2)) + (configs.scalar_reg/2) )/2)*2 : opvals.sdest;
...
void instruction_smrd(int opcode) {
	...
	instr.smrd.sdst = opvals.sdest % (configs.scalar_reg/2) + configs.scalar_reg/2;
