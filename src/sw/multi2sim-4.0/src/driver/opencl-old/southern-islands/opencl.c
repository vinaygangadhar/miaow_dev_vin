...
void si_opencl_clEnqueueNDRangeKernel_wakeup(struct x86_ctx_t *ctx, void *data)
{
	...
	if (unit_test_data != 0)
	{
		...
		unsigned char input_data_read[20];
		int input_data_address = 0;

		fgets(input_data_read, 20, unit_test_data);
		input_data_address = strtol((char*)input_data_read + 1, NULL, 16);
		...
		//if ((input_data_address + input_data_len) <= si_emu->global_mem_top)
		{
			mem_write(si_emu->global_mem, input_data_address, input_data_len, input_data_buf);
		}
		si_emu->global_mem_top = input_data_address + input_data_len;
	}
	/*MIAOW stop */
	...
	unsigned char* buf = xcalloc(si_emu->global_mem_top, sizeof(unsigned char));
	mem_read(si_emu->global_mem, 0, si_emu->global_mem_top, buf);

	for (int mem_count = 0; mem_count < si_emu->global_mem_top; mem_count++)
	{
		fprintf(datafp,"%.2x\n", buf[mem_count]);
	}
	free(buf);
