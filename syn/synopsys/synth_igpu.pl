...
$module_hash_ref =
{
  ...
  "reg_512x32b_3r_3w" =>
  {
    ...
    "hierarchy" => "gpu_tb/DUT[0]/sgpr0/sgpr_reg_file"
  },
  "reg_1024x32b_3r_2w" =>
  {
    "children" =>
    ...
    "hierarchy" => "gpu_tb/DUT[0]/vgpr0/reg_file/page[0]"
  },
  "reg_128x32b_3r_3w" =>
  {
    "hierarchy" => "gpu_tb/DUT[0]/sgpr0/sgpr_reg_file/bank0",
    "cactii_power" => "384.61498",
    "cactii_area" => "23604.464"
  },
  "reg_256x32b_3r_2w" =>
  {
    "hierarchy" => "gpu_tb/DUT[0]/vgpr0/reg_file/page[0]/bank0",
    "cactii_power" => "554.51488",
    "cactii_area" => "31328.584"
  }
};
########################################################
...
if(0 == $skiprtlbuild)
{
  `cd ../../src/igpu_verilog/tb && make clean && make build`;
}
########################################################
...
@part_of_list = `cd $outdir && ls ../../../src/igpu_verilog/rtl/*/build/*.v | grep -v "/rtl/tracemon/build/" | grep -v "/rtl/memory/build/" | grep -v "/rtl/instr_buffer/build/" | grep "_definitions.v" | sed -e 's/\\\(.*\\\)/& \\\\/g'`;
#chomp(@part_of_list);
@common_file_list = (@common_file_list, @part_of_list);

#Now push all verilog files other than definitions
@part_of_list = `cd $outdir && ls ../../../src/igpu_verilog/rtl/*/build/*.v | grep -v "/rtl/tracemon/build/" | grep -v "/rtl/memory/build/" | grep -v "/rtl/instr_buffer/build/" | grep -v "_definitions.v" | sed -e 's/\\\(.*\\\)/& \\\\/g'`;
#chomp(@part_of_list);
@common_file_list = (@common_file_list, @part_of_list);
