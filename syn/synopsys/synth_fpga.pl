...
if(0 == $skiprtlbuild)
{
  `cd ../../src/fpga_verilog/tb && make clean && make build`;
}
########################################################
...
@part_of_list = `cd $outdir && ls ../../../src/fpga_verilog/rtl/*/build/*.v | grep -v "/rtl/tracemon/build/" | grep -v "/rtl/memory/build/" | grep -v "/rtl/instr_buffer/build/" | grep "_definitions.v" | sed -e 's/\\\(.*\\\)/& \\\\/g'`;
#chomp(@part_of_list);
@common_file_list = (@common_file_list, @part_of_list);

#Now push all verilog files other than definitions
@part_of_list = `cd $outdir && ls ../../../src/fpga_verilog/rtl/*/build/*.v | grep -v "/rtl/tracemon/build/" | grep -v "/rtl/memory/build/" | grep -v "/rtl/instr_buffer/build/" | grep -v "_definitions.v" | sed -e 's/\\\(.*\\\)/& \\\\/g'`;
#chomp(@part_of_list);
@common_file_list = (@common_file_list, @part_of_list);
